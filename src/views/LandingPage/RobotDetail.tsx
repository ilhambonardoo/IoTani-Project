...
// --- 1. Komponen Loader yang Lebih Cantik ---
const Loader = () => {
...
const RobotViewer = () => {
  ...
  // Separate effect to handle scroll prevention after canvas is ready
  useEffect(() => {
    if (!ready) return;
    ...
    // Prevent window scroll from affecting OrbitControls
  ...
  return (
    // --- Container Luar (Padding & Background) ---
    <section className="w-full py-8 sm:py-10 px-4 sm:px-6 md:px-8 flex justify-center  bg-white">
      <div className="w-full max-w-5xl">
        ...
        >
          <div>
            <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-neutral-800">
              Prototipe
            </h2>
        ...
        <div
          ref={canvasRef}
          className="relative w-full h-[400px] sm:h-[500px] md:h-[600px] lg:h-[700px] xl:h-[800px] rounded-xl sm:rounded-2xl overflow-hidden shadow-lg border border-neutral-200"
          style={{
        ...
        {isMobile && ready && (
          <motion.div
            className="mt-4 p-3 sm:p-4 bg-blue-50 border border-blue-200 rounded-lg"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 1 }}
          >
            <p className="text-xs sm:text-sm text-blue-700 text-center">
              <span className="font-semibold">💡 Tip:</span> Gunakan dua jari
